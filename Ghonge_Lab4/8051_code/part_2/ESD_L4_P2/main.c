...
volatile char one_tenth_second='0'; // counter to initialize the one tenth of second
volatile char second_one='0'; //// counter to initialize the second
volatile char second_two='0';
volatile char minute_one='0';// counter to initialize the minute
...
void lcdinit(void);
/*Polls the LCD busy flag. Function does not return // until the LCD controller is ready to accept another command*/
void lcdbusywait(void);
/*Description: Sets the cursor to the specified LCD DDRAM address. // Should call lcdbusywait().*/
void lcdgotoaddr(unsigned char addr);
void lcdgotoxy(unsigned char row, unsigned char column);
/*Description: Writes the specified character to the current // LCD cursor position. Should call lcdbusywait().*/
...
void lcdgotoxy(unsigned char row, unsigned char column){
    ...
    if(row=='0') // taking the row and converting to address
        address=(0<<4);
    else if(row=='1') //for row 1
        address=(4<<4);
    else if(row=='2') //for row 2
        address=(1<<4);
    else
        address=(5<<4);

    if(row>'2' && column>='A' && column<='F') // value of column between A and F
        address|=('8'-'0'); // address or with substraction with 8
    else if(column>='A' && column<='F') // column bewtween A and F
        address|=(column-'7');
    else if(column>='0' && column<='9') // column between  0 and 9
        address|=(column-'0');
    else
        address|=('8'-48);

    lcdgotoaddr(address); // got to that address specified in above calculation
    // Poll for BF=0
    lcdbusywait(); // wait for LCD to be cleared
}
/*
 * lcdputch function
 * Parameters:
...
void lcdputch(char cc){
    if(get_cursor_address()==0x0F){
        ...
        //delay(50);
        // send command
        lcd_ptr=cc; //pass the command character cc
        // Poll for BF=0
        lcdbusywait();

        lcdgotoxy('1','0'); // print it on lcd position 1,0
    }
...
void init_timer0(void){
    TCON=TCON&(~0x30); // initialize the TCON register
    TMOD=0x01; // set the mode for timer 0
    TL0=0x00; // set the low value
    TH0=0xF0; // set the high value of timer
    IE=0x82; // initalize the interrupt
    TCON|=0x10;
